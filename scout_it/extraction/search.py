"""Search engines and DDGS helpers - EnterpriseSearchEngine, ImageSearchEngine.

Extracted from the original monolithic extraction.py during the modular split.
Re-exports the DDGS package for backward compatibility.
"""
    ...
    def _phase_content_extraction(self):
        ...
        def extract_worker(result: EnterpriseResult) -> EnterpriseResult:
            ...
            try:
                ...
                # Skip the fallback chain when an error page was detected so a
                # dead-link/404 page is not "rescued" back into real content.
                if not error_page_detected and len(main_content.strip()) < 30:
